...
| `parts/acmref.typ` | pure-Typst port of `ACM-Reference-Format.bst` (output state machine + `format.*` helpers + per-type handlers + crossref resolution + numeric & author-year cite/sort layer + DOI/URL/arXiv hyperlinks); driven by native `@key`/`#cite`/`#bibliography` (via `show ref:`/`show cite:`/`show bibliography:` rules in `lib.typ`) or the equivalent exported `acm-cite`/`acm-citet`/`acm-citeyear`/`acm-citeauthor` / `acm-bibliography`. Reproduces the `.bst`'s reference text exactly — gated by the `bib-all`/`bib-edge`/`crossref`/`authoryear`/`mathfields`/`keycite` twins |
