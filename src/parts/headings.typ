...
#let heading-number(pattern, loc) = {
  if pattern != none { numbering(pattern, ..counter(heading).at(loc)) }
}
...
#let render-heading(it, cfg) = {
  ...
  context {
    ...
    let num = if lvl <= cfg.secnumdepth { heading-number(pattern, loc) }

    // The query includes this heading; the preceding one is at -2.
    let prevh = query(selector(heading).before(loc)).at(-2, default: none)
    let body-since = _body-since-heading.at(loc)
    ...
    let suppress-before = outlined and prevh != none and prevh.outlined and prevh.level <= 2 and not body-since
    ...
    if lvl <= 2 {
      ...
      run-in-heading(title, cfg, sec-font(cfg, "subsubsection"),
      ...
      run-in-heading(title, cfg, sec-font(cfg, "paragraph"),
        before: tex-skip(cfg, if suppress-before { 0.25 * bls } else { 0.5 * bls }),
        stretch: if suppress-before { 0pt } else { heading-stretch },
        indent: cfg.parindent, ambient: ambient, num: none)
    } else {
      run-in-heading(title, cfg,
