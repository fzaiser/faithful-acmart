...
A report that was always a report gets none #cite("TyReportNone"), and free text is printed as it stands #cite("TyFree"), including a value the _.bst_ backend would read as a missing-value marker #cite("TyMarker"), and a title that reads as one still labels its own citation #cite("TyMarkerLabel").
