%% biblatex-fields-test - BibLaTeX field formats under acmauthoryear.
\documentclass[acmsmall,nonacm,natbib=false]{acmart}
\pagestyle{empty}
%% acmart's standardpagestyle puts \shorttitle in the odd-page running head, and
%% amsart only defines it via \title. This titleless fixture runs past page 2, so
%% give it the empty \shorttitle a \title{} would (the Typst twin has no title either).
\def\shorttitle{}
...
\addbibresource{biblatex-fields-test.bib}
\begin{document}
\thispagestyle{empty}
...
A report that was always a report gets none \cite{TyReportNone}, and free text is printed as it stands \cite{TyFree}, including a value the \emph{.bst} backend would read as a missing-value marker \cite{TyMarker}, and a title that reads as one still labels its own citation \cite{TyMarkerLabel}.
...
\section*{Name lists}
A reference list shows at most nine names \cite{MbNine} before falling back to the first one and \emph{et al.} \cite{MbTen,MbTenEditors}, which an explicit \emph{and others} does not count towards \cite{MbNineOthers}.

\section*{Sentence casing}
...
\section*{Truncated name lists}
A name list cut down to \emph{et al.} ends in a full stop of its own, whichever list it is: a dataset's \cite{TrDataset}, a translator's \cite{TrTranslator}, a patent holder's \cite{TrHolder} or a book author's \cite{TrBookauthor}.

\section*{Cite labels}
Only the author-year style disambiguates a cite label, so it alone tells two authors of a surname apart \citeauthor{UnSmithJohn} \citeauthor{UnSmithJane}, widens a name list past the truncation point \citeauthor{UnListA} \citeauthor{UnListB}, and drops a name prefix that the numeric style keeps \citeauthor{UnPrefix}.
