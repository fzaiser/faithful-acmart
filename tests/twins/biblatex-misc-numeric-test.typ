...
A legacy spelling renames rather than overwrites, so an entry that carries both keeps the canonical one #cite("TwoSpellings"), and a proceedings entry prints every stage its own driver has #cite("FullProc").
