...
#let sample-abstract = [
  A clear and well-documented #latex-logo document is presented as an article
...
#latex-logo style for use across ACM publications, and incorporates accessibility and
metadata-extraction functionality necessary for future Digital Library
endeavors. Numerous ACM and SIG-specific #latex-logo templates have been examined, and
...
information, the _#latex-logo User's Guide_ is available from
...
typeface family. Your #tex-logo installation should include this set of packages.
...
the completed rights form once it has been submitted. This form contains #latex-logo
...
and generate the commands to be included in the #latex-logo source.
...
Your work should use standard #latex-logo sectioning commands: #raw("\\section"),
...
*tabular* material are found in the _#latex-logo User's Guide._
...
in #latex-logo #cit("Lamport:LaTeX")\; this section will simply show a few examples of in-text
...
available in #latex-logo\; this section will just give a couple of examples of display
...
just to demonstrate #latex-logo's able handling of numbering.
...
The use of #bibtex-logo for the preparation and formatting of one's references is
...
article DOI, etc.
  ...
  The command `\addbibresource{bibfile}` declares the #bibtex-logo file to use in the
  *preamble* (before the command "`\begin{document}`") of your #latex-logo source where
  ...
  "`\begin{document}`") of your #latex-logo source.
] else [
  ...
  command "#raw("\\begin{document}")") of your #latex-logo source:
  ```
